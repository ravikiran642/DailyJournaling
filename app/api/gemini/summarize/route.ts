...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingHash =
      typeof data.existingHash === 'string'
        ? data.existingHash
        : typeof data.existingEmbeddingSourceHash === 'string'
        ? data.existingEmbeddingSourceHash
        : typeof data.entry?.embeddingSourceHash === 'string'
        ? data.entry.embeddingSourceHash
        : '';
    const existingEmbedding: number[] = Array.isArray(data.existingEmbedding)
      ? data.existingEmbedding
      : Array.isArray(data.entry?.embedding)
...
${journalText}`;
    ...
    let embeddingValues: number[] = existingEmbedding;
    // Call generateEmbeddingWithFallback only when the existing hash from the db is different than the current hash generated by semanticText
